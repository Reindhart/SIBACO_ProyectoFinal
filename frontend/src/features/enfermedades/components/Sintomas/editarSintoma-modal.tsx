...
import type { Symptom } from '../sintomas-tab'

type EditSymptomModalProps = {
  symptom: Symptom
  onClose: () => void
  onSave: () => void
}

export default function EditSymptomModal({ symptom, onClose, onSave }: EditSymptomModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.put(`/api/symptoms/${symptom.id}`, formData)
      
      onSave()
    } catch (err: any) {
      setError(err.message || 'Error al actualizar el síntoma')
      ...
      <div className="modal-box max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-xl">Editar Síntoma</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Sección: Información del Síntoma */}
