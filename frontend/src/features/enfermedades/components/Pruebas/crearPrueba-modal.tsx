...
export default function CreateTestModal({ onClose, onSave }: CreateTestModalProps) {
  const [testType, setTestType] = useState<'lab' | 'postmortem'>('lab')
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setTestType(e.target.value as 'lab' | 'postmortem')
    setFormData(prev => ({
      ...prev,
      code: '',
      normal_range: '',
  ...
  // Cargar lista de enfermedades para el combo
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiClient.get(`/api/diseases?page=1&page_size=1000`)
        // Respuesta esperada: { status, data: Disease[], pagination }
  const items = ((res as any)?.data || []) as Array<any>
        setDiseases(items.map(d => ({ code: d.code, name: d.name })))
      } catch (err) {
        // no bloquear al usuario si falla la carga de enfermedades
        console.error('Error cargando enfermedades para dropdown:', err)
      }
    }
    load()
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (testType === 'postmortem' && !formData.death_cause) {
    ...
    try {
      ...
      const endpoint = testType === 'lab' ? '/api/lab-tests' : '/api/postmortem-tests'
      const dataToSend = testType === 'lab'
        ? {
            code: formData.code,
            name: formData.name,
      ...
      <div className="modal-box max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-xl">Nueva Prueba</h3>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <fieldset className="m-0">
                  <legend className="font-semibold block mb-1">Tipo de Prueba:</legend>
                  <select
